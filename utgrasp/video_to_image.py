...
def vid_to_images(video_file, output_dir, skip_frames = 0):
    ...
    while(cap.isOpened()):
        ...
        if frame % (skip_frames + 1) == 0:
            image_filename = "{}_{:08d}.jpg".format(video_name, frame + 1)
...
if __name__ == '__main__':
    ...
    if args.video_dir:
        video_files = [os.path.join(args.video_dir, v) for v in os.listdir(args.video_dir) if v.endswith('.MP4') or v.endswith('.mp4') or v.endswith('.mpg')]
