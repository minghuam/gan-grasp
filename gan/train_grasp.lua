...
opt = lapp[[
  -s,--save          (default "logs512_grasp")      subdirectory to save logs
...
local lfwHd5 = hdf5.open('grasp_data/grasp.hdf5', 'r')
local data = lfwHd5:read('grasp'):all()
print(data:size())
data:mul(2):add(-1)
lfwHd5:close()
...
function getSamples(dataset, N)
  ...
  local to_plot = {}
  for i=1,N do
    -- to_plot[#to_plot+1] = samples[i]:float():add(mean[1])
...
while true do
  ...
  formatted:float()
  image.save(opt.save .."/v1_"..(epoch or 0)..'.png', formatted)
  if opt.gpu then
    torch.setdefaulttensortype('torch.CudaTensor')
  else
    torch.setdefaulttensortype('torch.FloatTensor')
